...
class TLModel(nn.Module):
    ...
    def _get_tombstones(self, argsort):
        ...
        ndynamic_buckets = 3   # atm the first few
    ...
    def evaluate(self, data, hidden, argsort, eos_tokens=None, dump_hiddens=False):
        ...
        while i < data.size(0)-1:
            ...
            print(all_words_times_W_i[self.buckets[bucket]:self.buckets[bucket]+bucket_size].size(), hidden_times_U[:bucket_size].size())
            ...
            if not eos_tokens is None and data[i].data.cpu().numpy()[0] in eos_tokens:
                ...
                hidden = output[idx - self.buckets[bucket]].view(1, 1, -1)
